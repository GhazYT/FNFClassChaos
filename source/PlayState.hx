...
class PlayState extends MusicBeatState
{
	/*
	List:
- Delete the vanilla difficulties and add Crazy and Insane

- Add a secret when you in the choose week, press ctrl+alt+x and there will be a cheat screen where you can enter a cheat code to get some milk(the classchaos currency) and unlock songs. Codes:
-ImFxkinC0ol 500 milk
-SuSsy 3000milk
-40m1lk unlocks Milk song(a cover)
- TitanicLove unlocks heartbass but the whole class sings it(cover too)

- Add a character select (i will send the example for it)

- Move the menu buttons to the left and add random character to the right side of the screen

- add a warning screen when the game starts. warning text: (Hey Player! Welcome! Before you enjoying some funky beats, please remember that this game has a lot of FLASHING LIGHTS! Now enjoy some funky beats!)

- Add achievements
1. (Name: Good Grade) (Requirements: Full combo week 1)
2. (Name: Excuse Me!?) (Requirements: Full combo the song Tug o War)
3. (Name: Uhh, I Thought We're Going To The Mall?) (Requirements: Hit 15 snacks note)
4. (Name: haha u suck at dis game) (Requirements: Get 150 misses at one song)
5. (Name: I Just Wanna Rap!!!!) (Requirements: Finish a song without idling)

- Add a note such as (Snacks Note, Book Note. If you hit the note, snacks or books will block the screen and notes)(Ice cream Note When you press it, each note will slow down i mean the scroll speed)

- Add a freeplay song types like golden apple such as: Original, Class Chaos, Funny Indonesian Song

- Change rating name
Rating names by lowest
Why
Oops
Nevermind
STOPP
...
Cutie Rapper 
	*/
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Freak', 0.5], //From 40% to 49%
		['Beginner', 0.6], //From 50% to 59%
		['Normal', 0.69], //From 60% to 68%
		['Advanced', 0.7], //69%
		['Pro', 0.8], //From 70% to 79%
		['Osu player', 0.9], // From 80% to 89%
		['Full Flippin\' Combo', 1], //From 80% to 89%
	];
	...
	public var characterprefix:String = '';
	...
	override public function create()
	{
		...
		if (isPixelStage)
		{
			characterprefix = '-pixel';
			trace('pixel stage');
		}
		else
		{
			trace('non-pixel stage');
		}

		if (SONG.forcecharcters)
		{
		...
		{
			boyfriend = new Boyfriend(0, 0, ClientPrefs.curCharacter + characterprefix);
			startCharacterPos(boyfriend);
			boyfriendGroup.add(boyfriend);
			startCharacterLua(boyfriend.curCharacter);
		}
		...
		var file:String = Paths.txt(ClientPrefs.addon + songName + '/' + songName + 'Dialogue'); //Checks for vanilla/Senpai dialogue
		if (OpenFlAssets.exists(file)) {
	...
	function startSong():Void
	{
		...
		FlxG.sound.playMusic(Paths.inst(ClientPrefs.addon + PlayState.SONG.song), 1, false);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (SONG.needsVoices)
		{
			vocals = new FlxSound().loadEmbedded(Paths.voices(ClientPrefs.addon + PlayState.SONG.song));
		}
		else
		{
			vocals = new FlxSound();
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(Paths.image(pixelShitPart1 + daRating + pixelShitPart2, true));
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'combo' + pixelShitPart2, true));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'num' + Std.int(i) + pixelShitPart2, true));
